...
public:
    ...
    int maxLivello(Nodo<H>* x){
        int d, s;
        if(x){
            d=maxLivello(x->getDx());
            s=maxLivello(x->getSx());
            if(s>d)
                return 1+s;
            else
                return 1+d;
    ...
    int livello(Nodo<H>* x, int curr, int desired){
        if(x==NULL)
            return 0;
        if(curr==desired)
            return 1;
        else return livello(x->getSx(), curr+1, desired) + livello(x->getDx(), curr+1, desired);
    }
    void livelli(){
        int h=maxLivello(radice);
        out << h-1 << " ";
        for(int i=0; i<h; i++){
            int tmp=livello(radice, 0, i);
            out << tmp << " ";
        }
        out << endl;
    }
